...
### D36. `quiz publish` — authored 와 published 를 나눈다

**결정** `content/authored/` 가 원본이고, `content/published/` 는 앱이 읽는 파생물이다.
`verified` 만 나가고 검수 메타(`review`/`source`/`status`/`standardCode`)는 제외한다 —
아이 폰으로 내려받는 데이터이므로 필요한 것만 보낸다.
학년별로 한 파일에 모은다 (`e6.json`). 한 아이는 한 학년만 보기 때문이다.

**published 를 커밋하는 이유** Vercel 은 `apps/study-buddy` 를 Root Directory 로 빌드하므로
`tools/quiz-gen` 을 돌리지 않는다. 커밋해 두면 배포에 추가 단계가 없다.
어긋남은 `pnpm quiz:check` 로 잡는다 (내보내지 않고 최신인지만 확인).

**알려진 확장 한계** 문제당 약 1.2KB 다. 학년당 900문제면 1MB, 전 학년 5,400문제면 6MB 로
모바일 PWA 한 번에 싣기엔 크다. 지금(34문제, 40KB)은 문제없다.
학년당 수백 문제를 넘어가면 **과목별 분리 또는 서버 조회**로 바꾼다.
그때 바꿔도 앱의 데이터 접근 계층만 고치면 되도록 다음 단계에서 그 계층을 분리한다.

### D37. `@labs/quiz-engine` 진입점을 둘로 나눴다

**문제** 앱에서 `@labs/quiz-engine` 을 그대로 import 하면 깨진다.
`passage.ts` 가 **`node:crypto`** 를 쓰고(브라우저에 없다), `question.ts` 가 **zod** 를 끌어온다(~50KB).

**결정**

| 진입점                      | 내용                                     | 쓰는 곳   |
| --------------------------- | ---------------------------------------- | --------- |
| `@labs/quiz-engine`         | 전부 (zod 스키마, 검증, 위험도, 지문 id) | `tools/*` |
| `@labs/quiz-engine/runtime` | 학년·과목·묶음 + **타입만 재수출**       | `apps/*`  |

앱은 런타임에 스키마 검증을 하지 않는다 — 발행(`quiz publish`) 때 이미 검증했으므로
같은 일을 아이 폰에서 다시 할 이유가 없다. 타입은 `import type` 으로 가져오면
컴파일 시 사라지므로 비용이 0 이다.

`dist/runtime.js` 의 import 를 재귀 추적해 `zod`/`node:*` 가 없음을 확인했다.

### D38. 지문 묶음 필터를 테스트 가능한 함수로 옮겼다

**계기** 앱에서 검증하다가 **내 검증이 허술했음을 발견했다.**
`all.find(q => q.passageId)` 로 묶음을 골랐는데 그것이 지문 공유 묶음이 아니라
**단독 예문 문제**여서, "묶음이 함께 빠지는가" 테스트가 의미 없이 통과했다.

**조치** 규칙을 `filterGroupAware(questions, predicate)` 로 quiz-engine 에 옮기고
테스트 4개를 붙였다. 앱의 `queryQuestions` 는 이 함수를 부르는 얇은 껍데기다.

**알려진 부수효과 (테스트로 고정함)**
난이도가 섞인 지문 묶음은 난이도 필터에서 **통째로 빠진다.**
실측: 34문제에서 난이도별 조회 합이 32 였다 (13+14+5) — 난이도 2·3 문제가 한 지문을 쓰기 때문.
"오늘의 5문제" 처럼 난이도를 좁히지 않는 흐름에서는 문제가 없다.

**교훈** 픽스처를 우연에 맡기면 통과하는 테스트가 아무것도 증명하지 않는다.
검증하려는 조건을 **명시적으로 구성**해야 한다.

### D39. Next.js 는 `_` 로 시작하는 폴더를 라우팅에서 제외한다

임시 검증 라우트를 `src/app/__datacheck/` 에 만들었더니 404 였다.
App Router 는 밑줄로 시작하는 폴더를 private 로 보고 라우팅하지 않는다.
검증용 라우트를 만들 때는 밑줄 없는 이름을 쓴다. 그리고 **삭제한 뒤에는
`.next` 를 지워야** 한다 — 생성된 타입(`.next/dev/types/validator.ts`)이 남아 typecheck 가 깨진다.
